 ...
 * <p/>
 * - [florin] - added support for utf-8 character encoding, gzip and for
 * obtaining a better performance. Added log4j support
 * <p/>
 * ================================================================= This
...
public class ProxyServlet extends HttpServlet {
			...
			if (followRedirects) {
				if (stringLocation.contains("jsessionid")) {
					...
					if (nameValue[0].equalsIgnoreCase("jsessionid")) {
						httpServletRequest.getSession().setAttribute("jsessionid" + this.getProxyHostAndPort(), nameValue[1]);
	...
	private void setProxyRequestCookies(HttpServletRequest httpServletRequest, HttpMethod httpMethodProxyRequest) {
		...
		if (httpServletRequest.getSession().getAttribute("jsessionid" + this.getProxyHostAndPort()) != null) {
			String jsessionid = (String) httpServletRequest.getSession().getAttribute("jsessionid" + this.getProxyHostAndPort());
